...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <div class="weibo_middle pull-left">
        <?php if($show_post): ?><div  class="weibo_content weibo-content-post weibo_post_box" id="send_box">
        <div class="weibo_content_p">
            <div class="send-top  row" id="input_tip">
                ...
                <ul class="release-tabs">
                    <li class="release-tab-item active"><a data-role="check-dynamic" href="javascript:;">动态</a></li>
                    <li class="release-tab-item"><a data-role="check-longarticle" href="javascript:;">长文章</a></li>
            ...
            <div class="loading">
                <div style="width: 80px;heihgt:80px;margin: 0 auto;"><img src="/yoyo/Application/Weibo/Static/images/loading-new.gif"></div>
            </div>

            <div style="display: none; " class="dynamic-edit-box">
                <div class="tare">
    ...
    <script>
         ...
         */
        var _FLANG = true;
        $(".release-tab-item").on("click","a",function () {
            var left=0;
            var element=$(this);
            var active_elementwidth=element.parent().innerWidth();
            var prevs=element.parent().prevAll();
            prevs.each(function () {
                left=left+$(this).innerWidth();
            });
            left=left+(active_elementwidth-40)/2;
            $(".slider").animate({
                left:left
            },200);
            var dataName=element.attr("data-role");
            element.parent().siblings().removeClass("active");
            element.parent().addClass("active");
            if(dataName === "check-longarticle"){
                $(".dynamic-edit-box").hide();

                if(_FLANG){
                    $(".abbreviations").hide();
                    $(".loading").show();
                    $(".long-edit-box").load(U("Weibo/Index/editBoxLong"),function () {
                        $(".loading").hide();
                        $("#longtitle")[0].focus() ;
                    });
                    _FLANG=false;
...
//                $(".edit-box").load(U("Weibo/Index/editBoxLong"));
            }
            if(dataName === "check-dynamic") {
                $(".long-edit-box").hide();
//                $(".abbreviations").show();
                dynamicShow();
            }
        });
        $(".abbreviations").on("click",function () {
            $(".abbreviations").hide();
            dynamicShow();
        })
        function dynamicShow() {
            $(".dynamic-edit-box").show();
            $("#weibo_content")[0].focus() ;
        }
        function longEditShow(){
            $(".long-edit-box").show();
            $("#longtitle")[0].focus() ;
        }
    </script>
    ...
    <link href="/yoyo/Public/js/ext/webuploader/css/webuploader.css" type="text/css" rel="stylesheet"><?php endif; ?>
        ...
        <div class="top-weibo clearfix">
            <?php if(is_array($top_list)): $i = 0; $__LIST__ = $top_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$top): $mod = ($i % 2 );++$i;?><div class="top-list">
                    <a href="<?php echo U('Weibo/Index/weibodetail',array('id'=>$top['weibo_id']));?>">
                        <?php if(($top["crowd_id"]) == "0"): ?><span class="lable">全站</span>
                            <?php else: ?>
                            <span class="lable">圈内</span><?php endif; ?>
                        <?php echo ($top["title"]); ?>
                    </a>
                    <a href="<?php echo ($top["weibo"]["user"]["space_url"]); ?>"><?php echo ($top["weibo"]["user"]["nickname"]); ?></a>
        ...
        <!--全文置顶 start-->
        <div id='top_list'>
            <?php if(is_array($top_lists)): $i = 0; $__LIST__ = $top_lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$top): $mod = ($i % 2 );++$i; echo W('WeiboDetail/detail',array('weibo_id'=>$top,'can_hide'=>1, 'crowdId'=>$crowd_type)); endforeach; endif; else: echo "" ;endif; ?>
        </div>
        <!--全文置顶 end-->
        <div id="weibo_list">
            <?php if($page != 1){ ?>
    ...
    <?php if(!empty($suggested_follows)): ?><div class="all-wrap main_visual" style="max-width: 680px;margin:10px 0;height: 160px">
            <h2>推荐用户</h2>
...
<!--热门话题排行end--><?php endif; ?>
    ...
    <div style="position: relative">
        ...
        <?php echo W('Common/Adv/render',array(array('name'=>'checkrank_bottom','type'=>1,'width'=>'300px','height'=>'100px','title'=>'签到下方广告')));?>
        <!--广告位end-->
    </div>
</div>
    <!--首页右侧部分结束-->

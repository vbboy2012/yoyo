...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <div class="col-xs-12">
        <div class="input-group input-group-lg" style="margin-left: 110px">
            <form id="search" action="/yoyo/coin/btc.html" method="get">
        ...
        <table class="table table-striped table-hover">
            ...
            <?php if(is_array($adList)): $i = 0; $__LIST__ = $adList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    <?php if($vo['driver'] == 'local'){ $img = substr($vo['path'],0,strlen($vo['path'])-4);$img.='_64_64.png'; $img = "/yoyo/Uploads/Avatar/".$img; }else{ $img = $vo['path']."/thumbnail/64x64!"; } $minPrice = number_format($vo['min_price']); $maxPrice = number_format($vo['max_price']); $price = number_format($vo['price'],2); $pay = query_pay($vo['pay_type']); $payName = ''; foreach ($pay as $item){ $payName .= $item['name'].','; } $payName = substr($payName,0,strlen($payName)-1); ?>
                    <td>
                        <a href="<?php echo U('Ucenter/index/information',array('uid'=>$vo['uid']));?>"><img src="<?php echo ($img); ?>" class="img-circle"> <?php echo ($vo["nickname"]); ?></a>
                    </td>
                    ...
                    <?php $params = ''; if($vo['type'] == 1){ $title = "购买"; $classBtn = "btn-primary"; $params = 'sellonline-'; }else if($vo['type'] == 2){ $title = "出售"; $classBtn = "btn-warning"; $params = 'buyonline-'; }else if($vo['type'] == 3){ $title = "购买"; $classBtn = "btn-primary"; $params = 'selllocal-'; }else if($vo['type'] == 4){ $title = "出售"; $classBtn = "btn-warning"; $params = 'buylocal-'; } $params.=$pay[0]['en_name']."-".$vo['countryEn']; ?>
                    <td style="vertical-align: middle"><a class="btn <?php echo ($classBtn); ?>" href="tradead/<?php echo ($vo["id"]); ?>/<?php echo ($params); ?>" target="_blank"><?php echo ($title); ?></a></td>

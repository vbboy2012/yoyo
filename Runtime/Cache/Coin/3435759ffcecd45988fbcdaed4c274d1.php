        ...
        <?php if(($order['type'] == 1 or $order['type'] == 3) and $order['get_uid'] == get_uid()): ?><p class="qtHead"><label><?php echo L('_TRADE_OPER_');?></label><label><span id="timer"></span></label></p>
            <div class="alert alert-primary" style="margin-top: 0px">
                <?php if($order['status'] == 1): ?><h4><?php echo L('_TRADE_STEP1_');?></h4>
                    ...
                    <hr>
                    <p><?php echo L('_TRADE_STEP3_INFO_',array('updateTime'=>$updateTime,'seller'=>$seller));?></p><?php endif; ?>
                <hr>
                <p><label><?php echo ($order["pay_remark"]); ?></label></p>
                <p>金额：<label><?php echo ($order["trade_price"]); ?> <?php echo ($order["currency"]); ?></label></p>
                <p>付款参考码：<label><?php echo ($order["pay_code"]); ?></label></p>
                <?php if($order['status'] == 1): ?><hr>
                    <h4><?php echo L('_TRADE_STEP2_');?></h4>
                    <hr>
                ...
                <hr>
                <a href="<?php echo U('/support/request/3/'.$order['order_id']);?>" target="_blank" class="btn btn-danger">申&nbsp;诉</a>
                <a href="javascript:void(0)" class="btn btn-default" id="cancel-trade" style="float: right">取消交易</a>
                <?php } ?>
            </div>
            ...
            <div class="alert alert-primary" style="margin-top: 0px">
                ...
                <?php if($order['status'] == 2): ?><hr>
                    <p>1，<?php echo L('_TRADE_STEP4_');?><br>2，<?php echo L('_TRADE_BUY_INFO3_');?></p>
                    <hr>
                    <a href="<?php echo U('/support/request/3/'.$order['order_id']);?>" target="_blank" class="btn btn-danger">申&nbsp;诉</a>
                    <a href="javascript:void(0)" class="btn btn-info" name="send-coin" style="float: right">放行比特币</a><?php endif; ?>
            </div>
            <?php if($order['status'] ==1 or $order['status'] ==2): ?><div class="alert alert-info" style="margin-top: 0px">
                    <p><?php echo L('_TRADE_TIPS1_');?></p>
                    <p><?php echo L('_TRADE_TIPS2_');?></p>

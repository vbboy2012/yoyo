...
<nav class="navbar navbar-default uc_navbar" role="navigation">
    ...
    <!-- /.navbar-collapse -->
                ...
                <div class="cont-wrap common-cont-wrap">
                    <?php if($user['email']){ $email = '<i class="icon icon-check-sign"></i> 已验证'; } if($user['mobile']){ $mobile = '<i class="icon icon-check-sign"></i> 已验证'; } ?>
                    <div class="col-xs-6">
                        <p>注册时间：<?php echo (date('Y-m-d',$user["reg_time"])); ?></p>
                        <p>语言：<?php echo ($user["lang"]); ?></p>
                        <p>电子邮件：<?php echo ($email); ?></p>
                        <p>电话号码：<?php echo ($mobile); ?></p>
                        <p>身份证认证：<?php echo ($mobile); ?></p>
                    </div>
                    <div class="col-xs-6">
                        <p>交易量：<?php echo ($user["btc_count"]); ?>BTC / <?php echo ($user["eth_count"]); ?>ETH</p>
                        <p>已确认的交易次数：<?php echo ($user["trade_count"]); ?></p>
                        <p>平均放行时间：</p>
                        <p>用户评价得分：<?php echo ($user["trade_score"]); ?>%</p>
                        <p>最后一次在线：<?php echo (date('Y-m-d h:i:s',$user["last_login_time"])); ?></p>
                    </div>
                </div>
            </div>
        </div>

...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <div class="col-xs-12 usercenter">
        <div class="uc">
            <div class="uc-cont">
                <div class="col-xs-2">
                    <?php if(ACTION_NAME=='myad'){ $tabClass['ad'] = 'active'; } elseif(ACTION_NAME=='mytrade'){ $tabClass['adtrade'] = 'active'; } else{ $tabClass[$type] = 'active'; } ?>
...
<nav class="navbar navbar-default uc_navbar" role="navigation">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
        <ul class="nav navbar-nav new-nav">
            <li>
                <a href="<?php echo U('coin/ad',array('uid'=>$user_info['uid']));?>" class="<?php echo ($tabClass["ad"]); ?>">我的广告</a>
                <a href="<?php echo U('mytrade',array('uid'=>$user_info['uid']));?>" class="<?php echo ($tabClass["adtrade"]); ?>">我的交易</a>
            </li>
                ...
                <div class="col-xs-10 common-cont-wrap">
                    <table class="table table-striped table-hover">
                        ...
                        <?php if(is_array($adList)): $i = 0; $__LIST__ = $adList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                <?php $img = substr($vo['path'],0,strlen($vo['path'])-4);$img.='_64_64'; $minPrice = number_format($vo['min_price']); $maxPrice = number_format($vo['max_price']); $price = number_format($vo['price'],2); if($vo['type'] == 1){ $type = '在线出售'; }else if($vo['type'] == 2){ $type = '在线购买'; }else if($vo['type'] == 3){ $type = '本地出售'; }else if($vo['type'] == 4){ $type = '本地购买'; } $coinType = $vo['coin_type'] == 1 ? 'BTC':'ETH'; $time = date('Y-m-d H:i:s', $vo['create_time']); $params = ''; if($vo['type'] == 1){ $params = 'sellonline-'; }else if($vo['type'] == 2){ $params = 'buyonline-'; }else if($vo['type'] == 3){ $params = 'selllocal-'; }else if($vo['type'] == 4){ $params = 'buylocal-'; } $params.=$vo['payEn']."-".$vo['countryEn']; ?>
                                <td style="vertical-align: middle;"><input type="hidden" value="<?php echo ($vo["id"]); ?>"><a href="/yoyo/ad/<?php echo ($vo["id"]); ?>/<?php echo ($params); ?>" style="color: #337ab7">#<?php echo ($vo["id"]); ?></a></td>
                                ...
                                <td style="vertical-align: middle"><?php echo ($vo["pre_price"]); ?>%</td>
                                <td style="vertical-align: middle"><?php echo ($time); ?></td>
                                <td style="vertical-align: middle;color: #0C7F12"><?php echo ($vo['status']==0?下架:激活); ?></td>
                                <td style="vertical-align: middle;"><a href="/yoyo/ad/advertise/<?php echo ($vo["id"]); ?>" target="_blank">编辑</a><a href="/yoyo/ucenter/index/myad/active/<?php echo ($vo["id"]); ?>/status/<?php echo ($vo["status"]); ?>"><?php echo ($vo['status']==0?激活:下架); ?></a><a id="delete" href="#">删除</a></td>

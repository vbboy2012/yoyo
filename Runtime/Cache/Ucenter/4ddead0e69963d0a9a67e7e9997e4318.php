...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <div class="v-wrap">
        <div class="v-head">
            认证中心
            <?php if(!empty($attest)): ?><a href="<?php echo U('Ucenter/Attest/process');?>">我的认证</a><?php endif; ?>
        </div>
        <div class="v-type row">
            <div class="row c-title">认证类型</div>
            <?php if(is_array($attest_type)): $i = 0; $__LIST__ = $attest_type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$one_type): $mod = ($i % 2 );++$i;?><div class="col-xs-4 type-box">
                    <div class="col-xs-5 box">
                        <div class="cover pull-right">
                            <a href="<?php echo U('Ucenter/attest/individual',array('id'=>$one_type['id']));?>">
                                <img src="/yoyo/Application/Ucenter/Static/images/attest/<?php echo ($one_type['name']); ?>.png" title="<?php echo ($one_type['title']); ?>" alt="">
                            </a>
                        </div>
                    </div>
                    <div class="col-xs-7 box info">
                        <p class="title"><a href="<?php echo U('Ucenter/attest/individual',array('id'=>$one_type['id']));?>"><?php echo ($one_type['title']); ?></a></p>
                        <p class="intro"><?php echo ($one_type['tips']); ?></p>
                        <?php if(empty($attest)): ?><a href="<?php echo U('Ucenter/attest/individual',array('id'=>$one_type['id']));?>" class="apply-btn">申请认证</a><?php endif; ?>
                    </div>
                </div><?php endforeach; endif; else: echo "" ;endif; ?>
        </div>
        <div class="v-famous">
            <div class="row c-title">名人堂</div>
            <?php if(is_array($celebrity)): $i = 0; $__LIST__ = $celebrity;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$one_celebrity): $mod = ($i % 2 );++$i;?><div class="col-xs-3 f-box">
                    <div class="col-xs-4 box">
                        <a href="<?php echo ($one_celebrity['user']['space_url']); ?>" ucard="<?php echo ($one_celebrity['uid']); ?>" target="_blank">
                            <?php echo ($one_celebrity['user']['avatar_html128']); ?>
                        </a>
                    </div>
                    <div class="col-xs-8 box">
                        <div class="info">
                            <p>
                                <a href="<?php echo ($one_celebrity['user']['space_url']); ?>" ucard="<?php echo ($one_celebrity['uid']); ?>" target="_blank">
                                    <?php echo ($one_celebrity['user']['nickname']); ?>
                                </a>
                            </p>
                            <p style="color: #666"><?php echo ((isset($one_celebrity['user']['signature']) && ($one_celebrity['user']['signature'] !== ""))?($one_celebrity['user']['signature']):'还没想好O(∩_∩)O'); ?></p>
                        </div>
                    </div>
                </div><?php endforeach; endif; else: echo "" ;endif; ?>
        </div>

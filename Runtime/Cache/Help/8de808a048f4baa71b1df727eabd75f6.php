...
<!--顶部导航之后的钩子，调用公告等 end-->
    ...
    <div class="white-popup1 boxShadowBorder col-xs-12" style="">
        <?php $email = session('ticket_send'); if($email){ $text = $email.L('_SEND_TICKET_TIPS_'); } ?>
        <?php if(is_login()): if($text){ ?>
                <div class="alert alert-success" style="margin-top: 0px"><?php echo ($text); ?></div>
            <?php } ?>
            <table class="table table-striped table-hover">
                ...
                <tbody>
                <?php if(is_array($ticket)): $i = 0; $__LIST__ = $ticket;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        <?php $typeText = L('_TICKET_QUESTION'.$vo['type'].'_'); $time = date('Y-m-d H:i:s', $vo['create_time']); if($vo['update_time']){ $update = date('Y-m-d H:i:s', $vo['update_time']); } ?>
                        <td style="vertical-align: middle;"><?php echo (md5($vo['id'])); ?></td>
                        <td style="vertical-align: middle;"><?php echo ($typeText); ?></td>
                        <td style="vertical-align: middle"><?php echo ($vo['content']); ?></td>
                        <td style="vertical-align: middle"><?php echo ($time); ?></td>
                        <td style="vertical-align: middle;color: #0C7F12"><?php echo ($update); ?></td>
                    </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                </tbody>
            </table>
            <?php else: ?>
            <?php if(!$text){ $text = L('_SEND_TICKET_TIPS2_'); } ?>
            <div class="alert alert-success" style="margin-top: 0px"><?php echo ($text); ?></div><?php endif; ?>
    </div>

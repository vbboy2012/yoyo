...
class QuestionAnswerModel extends Model
{
    ...
    public function addAnswer($uid, $question_id, $content)
    {
    ...
    {
        $answer=$this->where(array('id'=>$id,'status'=>1))->find();
        $answer['content'] = $this->parseComment($answer['content']);
        $answer['user'] = query_user(array('uid', 'avatar32', 'avatar64', 'avatar128', 'space_url', 'nickname'), $answer['uid']);
        $answer['content'] = parse_at_users($answer['content'], true);
        $answer['content'] = parse_emoji($answer['content']);
        $answer['create_time'] = friendlyDate($answer['create_time']);
        $best=D('question')->where(array('id'=>$answer['question_id']))->find();
        $answer['best_answer_id']=$best['best_answer'];
        $answer['who_ask']=$best['uid'];

        $support['appname'] = 'question-answer';
        $support['table'] = 'question';
        $support['row'] = $id;
        $answer['support_all_count'] = D('Support')->where($support)->count();
        $support['uid'] = is_login();
        $answer['support_count'] = D('Support')->where($support)->count();
        if ($answer['support_count']) {
            $answer['is_support'] = '1';
        } else {
            $answer['is_support'] = '0';
        }

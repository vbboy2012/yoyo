...
function check_auth($rule = '', $except_uid = -1, $type = AuthRuleModel::RULE_URL)
{
    ...
    // 检测是否有该权限
    if (!M('m_auth_rule')->where(array('name' => $rule, 'status' => 1))->find()) {
        return false;
    }
    static $Auth = null;
    if (!$Auth) {
...
function render_picture_path($path)
{
    ...
    return is_bool(strpos($path, 'http://')) ? 'http://' . str_replace('//', '/', $_SERVER['HTTP_HOST'] . __ROOT__ . $path) : $path;
}
...
function parse_expression_callback($data)
{
    ...
    $parent_dir = '' ;
    if (is_dir('../Uploads/Expression')&&!strpos( $info['src'], '://')) {
        $parent_dir = '..' ;
    }
    if ($info) {
        if ($key==0) {
            ...
            return preg_replace("/\\[.+?\\]/i", "<div class='expression_pic'><img class='expimg' src='".$parent_dir. $info['src'] . "' /></div>", $data[0]);
        }
...
/**
 * 清理OS的缓存 用法与S()一毛一样，只是简单的封装了下
 * @param $name
 * @param string $value
 * @param null $options
 * @return mixed
 */
function MS($name, $value = '', $options = null)
{
    S(array(
            'prefix'=>'',
        )
        ...
        foreach ($topics as $e) {
            if ($e['status'] == -1||$no_link) {
                ...
                if (modC('HIGH_LIGHT_TOPIC', 1, 'Weibo')) {
                    $content = str_replace("[topic:" . $e['id'] . "]", " <a class='label-info'  href=\"$space_url\">#" . $e['name'] . "# </a> ", $content);
                } else {
                    $content = str_replace("[topic:" . $e['id'] . "]", " <a href=\"$space_url\" >" . $e['name'] . " </a> ", $content);
                }
            }
        }
    }

 ...
 * @param string $type 取os还是v5mobile的配置，默认v5mobile
 * @return string
 * @auth 陈一枭
 */
function modC($key, $default = '', $module = '',$type = 'm')
{
    $mod = $module ? $module : MODULE_NAME;
    $type = $type == 'm' ? 'MConfig' : 'Config';
    if (MODULE_NAME == "Install" && $key == "NOW_THEME") {
        return $default;
    }
    $tag = $type.'conf_' . strtoupper($mod) . '_' . strtoupper($key);
    $result = S($tag);
    if ($result === false) {
        $config = M($type)->field('value')->where(array('name' => '_' . strtoupper($mod) . '_' . strtoupper($key)))->find();
        if (!$config) {
            $result = $default;
        } else {
            $result = $config['value'];
...
function os_modC($key, $default = '', $module = ''){
    return modC($key, $default , $module ,'os');
}

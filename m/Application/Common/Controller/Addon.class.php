...
abstract class Addon{
    ...
    final public function checkInfo(){
     ...
     public final function getOriginConfig($name=''){
        return $this->getConfig($name,'Addons');
    }
    /**
     * 获取插件的配置数组
     */
    final public function getConfig($name='',$addonsTable='MAddons'){
        if(empty($name)){
            $name = $this->getName();
        }

        $tag='addons_config_'.$addonsTable.'_'.$name;
        $config=S($tag);
        if($config===false){
            ...
            $map['name']    =   $name;
            $map['status']  =   1;
            $config  =   M($addonsTable)->where($map)->getField('config');
            if($config){

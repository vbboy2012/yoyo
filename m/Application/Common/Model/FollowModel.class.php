...
class FollowModel extends Model
{
    ...
    public function follow($uid)
    {
        ...
        D('Member')->where(array('uid'=>$uid))->setInc('fans');
        S('fans_rank',null);
        D('Common/Message')->sendMessage($uid, L('_FANS_NUMBER_INCREASED_'), $user['nickname'] . L('_CONCERN_YOU_WITH_PERIOD_'), 'Ucenter/Index/mine', array('uid' => is_login()),-1,'Ucenter');
    ...
    public function unfollow($uid)
    {
        ...
        D('Message')->sendMessage($uid, L('_NUMBER_OF_FANS_'), $user['nickname'] . L('_CANCEL_YOUR_ATTENTION_WITH_PERIOD_'), 'Ucenter/Index/mine', array('uid' => is_login()),-1,'Ucenter');
        D('Member')->where(array('uid'=>$uid))->setDec('fans');
        S('fans_rank',null);
    ...
    public function addFollow($who_follow, $follow_who, $invite = 0)
    {
        ...
        D('Common/Message')->sendMessage($follow_who, L('_FANS_NUMBER_INCREASED_'), $content, 'Ucenter/Index/mine', array('uid' => $who_follow), $who_follow,'Ucenter');
     ...
     */
    public function eachFriend($uid, $puid) {
        $uidFollow = $this->isFollow($uid, $puid) ;
        $puidFollow = $this->isFollow($puid, $uid) ;
        if($uidFollow && $puidFollow)
            return true ;
        return false;
    }
} 
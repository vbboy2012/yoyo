...
//        }
    ...
    //修改昵称 性别 签名 生日
    public function changeUserData($type,$uid,$value){
        if($type=='nickname'){
            $this->checkNickname($value);
            $this->checkDenyNickname($value);
            $this->checkNicknameLength($value);
        }
        $res=$this->where(array('uid'=>$uid))->setField($type,$value);
        if($res!==false){
            return true;
        }else{
            return false;
    ...
    //修改地区
    private function getPosId($name,$upid=0){
        if($upid){
            $map['upid']=$upid;
        }
        $map['name']=array('like',$name.'%');

         $id=M('district')->where($map)->getField('id');
         if(empty($id)){
             $ids=M('district')->where(array('upid'=>$upid))->field('id')->find();
             $set['upid']=array('in',$ids['id']);
             $set['name']=array('like',$name.'%');
             $id=M('district')->where($set)->getField('id');
         }
        return $id;
    }
    public function handleDistrict($pos){
        $tag_pro=$pos[0];
        $provinceId=S($tag_pro);
        if(empty($provinceId)){
            $provinceId=$this->getPosId($pos[0],0);
            S($tag_pro,$provinceId,60*60*24*30);
        }
        $tag_city=$pos[1];
        $cityId=S($tag_city);
        if(empty($cityId)){
            $cityId=$this->getPosId($pos[1],$provinceId);
            S($tag_city,$cityId,60*60*24*30);
        }
        if($pos[2]){
            $tag_dis=$pos[2];
            $disId=S($tag_dis);
            if(empty($disId)){
                $disId=$this->getPosId($pos[2],$cityId);
                S($tag_dis,$disId,60*60*24*30);
            }
        }

        return array($provinceId,$cityId,$disId);
    }
    public function changePos($uid,$pos){

        $data['pos_province']=$pos[0];
        $data['pos_city']=$pos[1];
        $data['pos_district']=$pos[2];
    ...
    public function getPeople($uid = '')
    {
        $people = S('people_by_'.$uid);
        if (empty($people)) {
            $people = $this->where(array('status'=>1,'uid'=>$uid))->find();
            S('people_by_'.$uid,$people,60*60*12);
        }
        return $people;
    }

    public function searchPeople($nickname = '',$page = 1 ,$count = 10)
    {
        $crowdAdmin = D('Weibo/WeiboCrowdMember')->getCrowdAllAdmin();
        $crowdAdmin = array_column($crowdAdmin,'uid');
        $admin = get_administrator();
        $adminList = $admin;
        $people = $this->field('uid')->where(array('nickname'=>array('like', '%' . $nickname . '%')))->page($page,$count)->select();
        $people = array_column($people,'uid');
        if (!empty($people)) {
            foreach ($people as &$v) {
                $v = $this->getPeople($v);
                ...
                if (in_array($v['uid'],$adminList)) {
                    $v['flag'] = '站长';
                } elseif (in_array($v['uid'],$crowdAdmin)) {
                    $v['flag'] = '圈主';
                } else {
                    $v['flag'] = '会员';
                }
                $v['user'] = query_user(array('avatar128','space_mob_url'),$v['uid']);
                empty($v['signature']) && $v['signature'] = "这个人很懒,神马都没写~";
            }
            unset($v);
        } else {
            return 'none';

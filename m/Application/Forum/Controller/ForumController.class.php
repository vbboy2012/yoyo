...
class ForumController extends AdminController
{
    ...
    public function config()
    {
        ...
        if (!$data) {
            ...
            $data['CACHE_TIME'] = 300;
            $data['ACTIVE_USER_TIME'] = 30;
        }


        $admin_config->title(L('_FORUM_SETTINGS_'))
            ->data($data)
//            ->keyInteger('LIMIT_IMAGE', L('_POST_PARSE_NUMBER_'), L('_POST_PARSE_NUMBER_VICE_'))->keyInteger('FORUM_BLOCK_SIZE', '论坛板块列表板块所占尺寸', '默认为4,，值可填1到12,共12块，数值代表每个板块所占块数，一行放3个板块则为4，一行放4个板块则为3')
            ->keyInteger('CACHE_TIME', L('_BLOCK_DATA_CACHE_TIME_'), L('_BLOCK_DATA_CACHE_TIME_DEFAULT_'))
            ->keyInteger('ACTIVE_USER_TIME', '论坛版块活跃用户时间范围', '30天')
...
//            ->keyText('FORUM_POST_CACHE_TIME', L('_BLOCK_SHOW_'), L('_BLOCK_SHOW_TIP_'))->keyDefault('FORUM_POST_CACHE_TIME', '600')
            ->group(L('_SETTINGS_BASIC_'), 'LIMIT_IMAGE,FORUM_BLOCK_SIZE,CACHE_TIME,SUGGESTION_POSTS,HOT_FORUM,RECOMMAND_FORUM,FORM_POST_SHOW_NUM_INDEX,FORM_POST_SHOW_NUM_PAGE,ACTIVE_USER_TIME');
...
//dump($forum1);exit;
    ...
    public function editPost($id = null,  $title = '', $content = '', $create_time = 0, $update_time = 0, $last_reply_time = 0, $is_top = 0)
    {
        ...
        if (IS_POST) {
            ...
            if ($isEdit) {
                $result = $model->where(array('id' => $id))->save($data);
                S('forum_detail'.$id,false);
                S('forum_detail_'.$id.'1',false);
            } else {
    ...
    public function replymanager($page = 1, $post_id = null, $r = 20)
    {
        ...
        //读取回复列表
            ...
            $list = $model->where($map)->order('create_time asc')->cache('forum_reply_manager'.$page,3600)->page($page, $r)->select();
    ...
    public function editReply($id = null, $content = '', $create_time = 0, $update_time = 0, $status = 1)
    {
        if (IS_POST) {
            ...
            if ($isEdit) {
                $result = $model->where(array('id' => $id))->save($data);
                S('forum_post_lzl'.$id,false);
                S('forum_lzl_reply_'.$id.'1',false);

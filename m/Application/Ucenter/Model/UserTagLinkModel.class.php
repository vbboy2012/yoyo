...
class UserTagLinkModel extends Model
{
    ...
    {
        $uid=is_login();
        $tags=$this->where(array('uid'=>$uid))->field('tags')->find();
        if(!empty($tags))
        {
            $arrTag=explode(',',$tags['tags']);
            foreach ($arrTag as $tag){
                $arr[]='%'.$tag.'%';
            }
            $map['tags']=array('like',$arr,'AND');
            $uids=$this->where($map)->field('uid')->order('uid asc')->limit(4)->select();
            return $uids;
        }else{
            return false;
        }

...
class MessageController extends AdminController
{
    ...
    public function userList($page=1,$r=20)
    {
        ...
        if (empty($aSearch1) && empty($aSearch2)) {
            ...
            foreach ($user as &$v) {
                $v['id'] = $v['uid'];
                $userData=query_user(array('mobile','email'),$v['uid']);
                $v['mobile']=$userData['mobile'];
        ...
        $builder->keyText('uid', '用户ID')->keyText('nickname', L('_"NICKNAME"_'))->keyText('mobile','手机号码')->keyText('email','邮箱');

...
class AuthManagerController extends AdminController
{
    ...
    public function deleteNode(){
        $aId=I('id',0,'intval');
        if($aId>0){
            $result=   M('MAuthRule')->where(array('id'=>$aId))->delete();
            if($result){
    ...
    private function getMergedRules($oldRules, $rules, $isAdmin = 'neq')
    {
        ...
        $otherRules = M('MAuthRule')->where($map)->field('id')->select();
    ...
    public function getNodeListFromModule($modules)
    {
        ...
        foreach ($modules as $module) {
            if ($module['is_setup']) {
                ...
                $node['child'] = M('MAuthRule')->where($map)->select();
                $node_list[] = $node;
            }

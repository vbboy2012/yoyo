...
class AdminController extends Controller
{
    ...
    public function getSubMenus($pid)
    {
        $menus = array();
        //生成child树
        $groups = M('m_menu')->where("pid = {$pid}")->distinct(true)->field("`group`")->order('sort asc')->select();
        ...
        $second_urls = M('m_menu')->where($where)->getField('id,url');
        ...
        foreach ($groups as $g) {
            ...
            $menuList = M('m_menu')->where($map)->field('id,pid,title,url,tip')->order('sort asc')->select();
    ...
    final public function getMenus($controller = CONTROLLER_NAME)
    {
        ...
        if ($menus === false) {
            ...
            $menus['main'] = M('m_menu')->where($where)->order('sort asc')->select();
            ...
            $current = M('m_menu')->where("url like '{$controller}/" . ACTION_NAME . "%' OR url like '%/{$controller}/" . ACTION_NAME . "%'  ")->field('id')->find();
            if ($current) {
                ...
                foreach ($menus['main'] as $key => $item) {
                    ...
                    if ($item['title'] == $nav_first_title) {
                        ...
                        //生成child树
                        $groups = M('MMenu')->where("pid = {$item['id']}")->distinct(true)->field("`group`")->order('sort asc')->select();
    ...
    final protected function returnNodes($tree = true)
    {
        ...
        if ((int)$tree) {
            $list = M('m_menu')->field('id,pid,title,url,tip,hide')->order('sort asc')->select();
            ...
            $nodes = M('m_menu')->field('title,url,tip,pid')->order('sort asc')->select();
            foreach ($nodes as $key => $value) {
                if (stripos($value['url'], MODULE_NAME) !== 0) {

...
class AddonsModel extends Model
{
    ...
    public function install($name)
    {
        ...
        session('m_addons_install_error', null);
        $install_flag = $addons->install();
        if (!$install_flag) {
            $this->error = L('_PERFORM_A_PLUG__IN__OPERATION_FAILED_') . session('m_addons_install_error');
            return false;
        }
        $addonsModel = D('MAddons');

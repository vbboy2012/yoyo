...
function allclear() {
    ...
    var url=U('Weibo/Crowd/allDelete');
...
function historyClick(content) {
    $("#search").val(content);
    var url=U('Weibo/Crowd/allSearch');
    $('.infinite-scroll-preloader').css('display', 'block') ;
    doSearch(content, 'no') ;
}
...
function doSearch(content, is) {
    ...
    $.post(url,{title:content,is:is},function (res) {
        if(res.status=="1"){
            $("[data-role='myList']").empty();
            if(res.html=="none"){
                $("[data-role='event-list']").html(" <div class='noWrap'>"+"<p class='emojiText'>╭(╯^╰)╮</p>"+
                    "<p class='noState open-about'>暂无搜索结果</p>"+" </div>");
            }
            else{
                $("[data-role='event-list']").html(res.html);
            }

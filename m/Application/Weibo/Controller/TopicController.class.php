...
class TopicController extends BaseController
{
    ...
    public function index()
    {
        ...
        $weibo_id=getSubByKey($list, 'weibo_id');

        $param['where']['id'] = array('in', $weibo_id);
        //排除私密圈子
        ...
        $list = D('Weibo/Weibo')->getWeiboList($param);
        foreach ($list as $val) {
            $detail = '' ;
            $detail = D('Weibo/Weibo')->getWeiboDetail($val);
            if ($detail) {
                $weibo[] = $detail ;
            }
        }
        ...
        $this->assignSelf();
        $this->setTitle('#'.$title.'#'.L('_LINE_LINE_').L('_TOPIC_'));
        if ($aIsPull) {
            $data['html'] = '';
            $data['status'] = 1;
            $data['html'] .= $this->fetch('Index/_list');
    ...
    public function topic()
    {
        ...
        list($topics,$totalCount) = D('Topic')->getHot($h, 10, 1);
    ...
    public function editTopic()
    {
        if (IS_POST) {
            ...
            $status=D('TopicFollow')->getFollowStatus($aId,is_login());
            $this->assign('host', $host);
            $this->assign('status', $status);
            $this->display() ;
        }
     ...
     */
    public function getRecommendTopicList () {
        if (IS_AJAX) {
            if (!is_login()) {
                $data['info'] = '请先登入帐号~' ;
                $data['status'] = -1 ;

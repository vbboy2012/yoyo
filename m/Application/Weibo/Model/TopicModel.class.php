...
class TopicModel extends Model
{
    ...
    public function getHot($hour = 1, $num = 10, $page = 1)
    {
        ...
        foreach ($list as $key=>&$v) {
            ...
            if ($topic['uadmin'] != 0) {
                $topic['user'] = query_user(array('space_link','nickname'), $topic['uadmin']);
            }
    ...
    public function getTopTopicList () {
        $tag='TOP_TOPIC_WEIBO_MOBILE';
        $list=S($tag);
        if($list===false){
            $list=$this->where(array('status'=>1,'is_top'=>1))->order('weibo_num desc')->limit(10)->select();
            S($tag,$list,60);
        }
        return $list;
    }

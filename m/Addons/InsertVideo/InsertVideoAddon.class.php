...
class InsertVideoAddon extends Addon
{
    ...
    public function parseExtra($extra = array()){
        ...
        if(preg_match("/.swf/i", $extra['video_url'], $check)){
            $this->error = '仅支持优酷、酷6、新浪、土豆网、搜狐、音悦台、腾讯、爱奇艺、哔哩哔哩等视频网址发布';
            return false;
        }

        if (!preg_match("/(youku.com|ku6.com|sohu.com|sina.com.cn|qq.com|tudou.com|yinyuetai.com|iqiyi.com|bilibili.com)/i",  $extra['video_url'], $hosts)) {
            $this->error = '仅支持优酷、酷6、新浪、土豆网、搜狐、音悦台、腾讯、爱奇艺、哔哩哔哩等视频网址发布';
            return false;
        }

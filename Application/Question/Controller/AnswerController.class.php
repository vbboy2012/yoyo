...
class AnswerController extends BaseController
{
    ...
    public function replyAnswer()
    {
        ...
        if (IS_POST) {
            ...
            $data['reply_id'] = I('post.reply_id', 0, 'intval');
            $question = $this->questionModel->getData($data['question_id']);
            $data['topic_id'] = $question['topic_id'];
            $answer = $this->questionAnswerModel->getData(array('id' => $data['reply_id']));
            if (mb_strlen($data['content'], 'utf-8') < modC('QUESTION_ANSWER_MIN_NUM', 20, 'Question')) {
                $this->error(L('_THE_ANSWER_IS_NO_LESS_THAN_') . modC('QUESTION_ANSWER_MIN_NUM', 20, 'Question') . L('_A_WORD_WITH_EXCLAMATION_'));
            }
            if (!in_array(is_login(),array($question['uid'],$answer['uid']))) {
                $this->error('回答失败,您无权限追问or追答~');
            }

            $res = $this->questionAnswerModel->editData($data);
            if ($res) {
                if (is_login() == $question['uid']) {
                    send_message($answer['uid'],'您有新的追问',get_nickname($question['uid']).'追问了关于【'.$question['title'].'】的问题','Question/Index/detailReply',array('id'=>$data['question_id'],'answer_id'=>$data['reply_id']),'Common_system');
                } else {
                    send_message($question['uid'],'您有新的追答',get_nickname($answer['uid']).'追答了关于【'.$question['title'].'】的问题','Question/Index/detailReply',array('id'=>$data['question_id'],'answer_id'=>$data['reply_id']),'Common_system');
                }
                $this->success('发送成功！',U('Question/Index/detailreply',array('id'=>$data['question_id'],'answer_id'=>$data['reply_id'])));
            } else {
                $this->error('发送失败！',U('Question/Index/detailreply',array('id'=>$data['question_id'],'answer_id'=>$data['reply_id'])));
            }
        }
    }

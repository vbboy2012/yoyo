...
class IndexController extends Controller
{
        ...
        if(D('Common/Module')->isInstalled('Mob')) {
            ...
            if(is_mobile() && ($sign == 0)) {
                if(ACTION_NAME!="detail") {
                    redirect(U('Mob/Forum/index'));
                }else{
                    $id = I('id','0','intval') ;
                    redirect(U('Mob/Forum/postDetail',array('id'=>$id)));
                }
            }
        }
    ...
    public function detail($id, $page = 1, $sr = null, $sp = 1)
    {
        ...
        unset($reply);
        //判断是否已经回复
        $is_reply=D('forum_post_reply')->where(array('post_id'=>$id,'uid'=>is_login()))->count();
        $this->assign('is_reply', $is_reply);
        //判断是否已经付费下载
        $post['pay_type_title']=D('ucenter_score_type')->where(array('id'=>$post['pay_type']))->getField('title');
        $download=D('file')->where(array('id'=>$post['file_id']))->find();
        $is_pay=D('forum_pay')->where(array('forum_post_id'=>$id,'uid'=>is_login()))->count();
    ...
    public function doEdit($post_id = null, $forum_id = 0, $title, $content)
    {
        ...
        $aOnHide = I('onHide', 0, 'intval');
        $aPayDownload = I('payDownload', 0, 'intval');
        $file_id = I('file_id', 0, 'intval');
        $pay_type = I('select_pay', 0, 'intval');
        $pay_num = I('pay_num', 0, 'intval');

        if($aPayDownload==1){
            if ($file_id==0){
                $this->error('请先上传付费的附件');
            }
            if ($pay_num<=0){
                $this->error('请选择正确的付费数值');
            }
        }
        ...
        $model = D('ForumPost');
        if ($isEdit) {
            $data = array('id' => intval($post_id), 'title' => $title,'hide'=>$aOnHide,'pay_on'=>$aPayDownload,'pay_type'=>$pay_type,'pay_num'=>$pay_num,'file_id'=>$file_id,'content' => $content, 'parse' => 0, 'forum_id' => intval($forum_id));
            ...
            $data = array('uid' => is_login(), 'title' => $title,'hide'=>$aOnHide,'pay_on'=>$aPayDownload,'pay_type'=>$pay_type,'pay_num'=>$pay_num,'file_id'=>$file_id,'content' => $content, 'parse' => 0, 'forum_id' => $forum_id);
    ...
    public function payDownload(){
        if (IS_POST){
            ...
            if ($success){
                $this->success('付费成功,页面即将跳转~',U('forum/index/detail', array('id' => $forum_id)));
            }else{
                $this->error('数据库存入失败');
            }
        }

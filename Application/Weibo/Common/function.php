 ...
 * @author:xjw129xjt(肖骏涛) xjt@ourstu.com
 */
function send_weibo($content, $type, $feed_data = '', $from = '', $pos = '', $crowdType = '')
{
    $topicModel = D('Weibo/Topic');
    $topicFollowModel = D('Weibo/TopicFollow');
    $uid = is_login();
    $weiboTopicLink = $topicModel->addTopic($content);
    $weibo_id = D('Weibo')->addWeibo($uid, $content, $type, $feed_data, $from, $pos, $crowdType);
    if (!$weibo_id) {
        return false;
    }
    ...
    D('Pushing')->sendMsg('all', array('uid' => $uid, 'weibo_id' => $weibo_id, 'crowd' => $crowdType), 'send_weibo');
...
function send_comment($weibo_id, $content, $comment_id = 0)
{
    ...
    if (!empty($weibo['crowd_id'])) {
        D('Weibo')->weiboNewReplyTime($weibo_id);
    }
...
function parse_crowd($crowd = 0)
{
    if (!empty($crowd)) {
        $res = D('Weibo/WeiboCrowd')->getCrowd($crowd);
        if ($res) {
            return $res['title'];
        }
    }
...
{
    $data = D('Weibo/WeiboCrowdMember')->getCrowdAdmin($crowd_id);
    if (empty($data)) {
        return -1;
    }
    return $data['uid'];

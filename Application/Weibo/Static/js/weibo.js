...
var send_weibo = function () {
    ...
    $(document).on('click','[data-role="send_weibo"]',function () {
        ...
        var pos = $(this).parents('.weibo_post_box').find('[name=pos]').val();
...
insert_image = {
    ...
    close: function () {
        if (typeof uploader_attach_ids != 'undefined') {
            uploader_attach_ids.off();
        }
...
var bind_receive_crowd = function () {
    ...
    $receive_member.click(function () {
        ...
        $.post(U('Weibo/Manage/receiveMember'), {uid: uid, crowd_id: crowd_id}, function (res) {
            handleAjax(res);
        });
    })
};
...
var bind_refuse_crowd = function () {
    ...
    $refuse_member.click(function () {
        ...
        var uid = $this.attr('data-uid');
        var crowd_id = $this.attr('data-crowd-id');
        $.post(U('Weibo/Manage/refuseMember'), {uid:uid, crowd_id: crowd_id}, function (res) {
            handleAjax(res);
        })
    })
}
...
var bind_remove_crowd_member = function () {
    ...
    $remove_member.click(function () {
        ...
        if (confirm('确定要移除该成员么？')) {
            ...
            var uid = $this.attr('data-uid');
            var crowd_id = $this.attr('data-crowd-id');
            $.post(U('Weibo/Manage/removeMember'), {uid: uid, crowd_id: crowd_id}, function (res) {
                if (res.status == 1) {
    ...
    $('.gcard').qtip({
        content: {
            text: function (event, api) {
                var crowd = $(this).attr('data-crowd-id');
                var href = U('Weibo/Index/index', ['crowd', crowd]);
                $.get(U('Weibo/Crowd/card'), {crowd: crowd}, function (res) {
                    console.log(res);
                    if (res.status == 1) {
                        var follow = '';
                        switch (res.info.is_follow) {
                            case 0:
                                follow = '<a href="javascript:" class="btn-follow" data-role="follow_crowd" data-id=' + res.info.id + '>+未加入</a>';
                                break;
                            case 1:
                                follow = '<a href="javascript:" class="btn-follow" data-role="unfollow_crowd" data-id=' + res.info.id + '>已加入 </a>';
                                break;
                            case 2:
                                follow = '<a href="javascript:" class="btn-follow" data-id=' + res.info.id + '>待审核 </a>';
                                break;
                        }
                        var html = '<div class="card-wrap one-circle"> ' +
                            '<div class="circle-cover-wrap">' +
                            '<a href=" ' + href + ' ">' +
                            '<img src="' + res.info.logo + '" alt="" title="社群封面">' +
                            '</a>' +
                            '</div>' +
                            '<div class="circle-info-wrap" style="width: 160px;">' +
                            '<a href=" ' + href + ' ">' +
                            '<div class="info-box-title">' + res.info.title + '</div>' +
                            '</a>' +
                            '<div class="info-box" style="color: #999">' + res.info.intro + '</div>' +
                            '<div class="info-box box-num"><span>成员' + res.info.member_count + '</span><span>讨论' + res.info.post_count + '</span></div>' +
                            '</div>' +
                            '<div class="circle-follow" style="margin-left: 10px">' +
                            follow +
                            '</a>' +
                            '</div>' +
                            '</div>';
        ...
        content: {
            text: function (event, api) {
                var html = '<div>' +
                    ...
                    '<div style="display: inline;margin-right: 35px;"><a style="text-align: center" onclick="collect(' + $(this).attr('data-id') + ')">加入收藏</a></div>' +
...
function collect(id) {
    $.post(U('Weibo/Index/CollectExpression'), {expression_id: id}, function (a) {
        handleAjax(a);
    });
}
...
if (MODULE_NAME == 'Weibo' && CONTROLLER_NAME == 'Index' && ACTION_NAME == 'index') {
    $(window).on('ws:send_weibo', function (obj, data) {
        ...
        var crowd = parseInt(weibo.crowd) || 0;
        if (crowd != 0 && (crowd != parseInt(data.content.crowd))) {
            return false
        }
        ...
        $('[data-role="show-new-weibo"]').click(function () {
            $('[data-role="show-new-weibo"]').unbind('click')
            $.post(U('weibo/index/getNewWeibo'), {count: parseInt($('.new-weibo-count').text()),crowd:crowd}, function (res) {
                $('#weibo_list').prepend(res.html);

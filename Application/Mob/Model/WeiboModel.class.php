...
class WeiboModel extends Model
{
    ...
    public function addWeibo($uid, $content = '', $type = 'feed', $feed_data = array(), $from = '', $crowdType='')
    {
//写入数据库

        //  dump($uid);exit;
        $create_time=time();
        $data = array('uid' => $uid, 'content' => $content, 'type' => $type, 'create_time'=>$create_time, 'data' => serialize($feed_data), 'from' => $from,'status'=>1,'crowd_id'=>$crowdType);

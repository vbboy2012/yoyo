...
class LocalCommentModel extends Model
{
    ...
    public function addNewsComment($uid, $aNewsId, $aContent, $comment_id = 0)
    {
        $issue="News";
        $mod="index";
        $time=time();
        ...
        //写入数据库
        $data = array('uid' => $uid,'app'=>$issue, 'mod'=>$mod,'content' => $aContent,'create_time'=>$time, 'row_id' => $aNewsId, 'comment_id' => $comment_id,'status'=>$status);
        $data = $this->create($data);
        if (!$data) return false;
        $comment_id = $this->add($data);

        //增加微博评论数量
        M('IssueContent')->where(array('id' => $aNewsId))->setInc('reply_count');

        S('IssueContent_' . $aNewsId, null);
        //返回评论编号
        return $comment_id;
    }

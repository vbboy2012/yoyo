...
class InviteModel extends Model
{
     ...
     */
    public function decNumber($uid=0,$type_id=0,$num=0)
    {
        $map['uid'] = $uid;
        $map['invite_type'] = $type_id;
        $res = M('InviteUserInfo')->where($map)->setDec('num',$num);//减少可邀请数目
        M('InviteUserInfo')->where($map)->setInc('already_num',$num);//增加已邀请数目
        return $res;
    }
     ...
     * @author 路飞<lf@ourstu.com>
     */
    public function createUserCode($uid = 0, $data = array(), $num = 1)
    {
        $map['status'] = 1;
        $map['id'] = $data['invite_type'];
        $invite_type = D('Ucenter/InviteType')->getSimpleList($map, 'length,time');

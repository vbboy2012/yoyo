...
window.onload=function () {
    ...
    switch(navtitle){
        case 'mygroup':
            $('#mygroup').addClass('cur');
            break;
        case 'discover':
            $('#discover').addClass('cur');
            break;
        case 'select':
            $('#select').addClass('cur');
            break;
...
function add_one_img() {
    var filechooser = document.getElementById("fileloadone");
    ...
    function upload(data) {
        ...
        $.post(dataUrl, {data: data}, function (msg) {
            if (msg.status == 1) {
                ...
                $("#logo_url").html('');
                $("#logo_url").html('<img src="' + msg.path + '"style="width:72px;height:72px"  data-role="issue_cover" >');
            } else {
                toast.error(msg.info);
            }
        }, 'json')

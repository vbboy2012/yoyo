...
class ShopController extends BaseController
{
    ...
    public function goodsBuy($id = 0, $name = '', $address = '', $zipcode = '', $phone = '', $address_id = '')
    {
        ...
        if ($goods) {
           ...
           $my_money=$my_money['score'.$score_type];


            if ((intval($my_money) - $money_need)<0) {
            ...
            if ($res) {
                ...
                $to_uids=get_auth_user('Admin/Shop/setGoodsBuyStatus');
                
                foreach ($to_uids as $u){
                D('Common/Message')->sendMessage($u, $content = '积分商城发货通知 ', $title = '买家已付款，快发货吧！', 'Admin/Shop/verify', array(), is_login());
              }
                //商城记录
                $shop_log['message'] = '用户[' . is_login() . ']' . get_nickname( is_login()) . '在' . time_format($data['createtime']) . '购买了商品<a href="index.php?s=/Shop/Index/goodsDetail/id/' . $goods['id'] . '.html" target="_black">' . $goods['goods_name'] . '</a>';

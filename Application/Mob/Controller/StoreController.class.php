...
//dump($goods);exit;
    ...
    public function goodsCate()
    {
        ...
        foreach ($cat as &$v) {
            $v['cat2'] = D('store_category')->where(array('pid' => $v['id'], 'status' => 1))->select();
        }
        $this->assign('cat', $cat);
//dump($cat);exit;
        $this->display();
    ...
    public function myCollect()
    {
        ...
        foreach ($goods as &$v) {
            $v['goods'] = D('StoreGoods')->where(array('id' => $v['info_id'],'status'=>1))->find();
        }
        unset($v);
        //dump($goods);exit;
        $this->assign('goods', $goods);
    ...
    public function delCollect()
    {
        ...
        $res=D('StoreFav')->where(array('fav_id'=>$aGoods_id))->delete();
        if($res){
...
//dump($sellOrder);exit;
    ...
    public function buy_add_item()
    {
        $item = $_POST;
        D('Cart')->addItem($item);
        $this->ajaxReturn(array('status' => 1, 'info' => '购买成功，转到结账页面。'));
    }

    /**权限检查
     * 范佳炜 fjw@ourstu.com
     */
    public function checkCustomAuth(){
          $this->ajaxReturn(check_auth('Store/Center/pay'));
    }

...
class ForumController extends BaseController
{
    public function _initialize()
    {
        ...
        if(is_login()){
                $this->_top_menu_list['right'][]=array('type'=>'edit','href'=>U('Mob/Forum/addpost'));
        }else{
    ...
    public function index($type = "all",$type_id="")
    {
        ...
        switch($type){
            case 'all':
                ...
                $this->assign('navtitle', 'all');
                $this->assign('forum', $forum);
                break;
            case 'hot':
                $suggestion_topics = modC('SUGGESTION_POSTS', '23,24,25,26,27', 'forum');
                $suggestion_topics = explode('|', $suggestion_topics);
                foreach ($suggestion_topics as $s) {
                    $post = M('ForumPost')->find($s);
                    $post['user'] = query_user(array('nickname', 'avatar32'), $post['uid']);
                    $post['summary'] = mb_substr(text($post['content']), 0, 80, 'utf8') . '...';
                    $forum[] = $post;
                }
                unset($s);
                $totalCount = D('ForumPost')->where(array('status' => 1))->count();
                $this->assign('navtitle', 'hot');
                $this->assign('forum', $forum);
                break;
            case 'top':
                $forum = D('ForumPost')->where(' status=1 AND is_top=' . TOP_ALL.' OR is_top='.TOP_FORUM)->order($order)->select();
                //读取帖子列表
                foreach ($forum as &$v) {
                    $v['user'] = query_user(array('nickname', 'avatar32'), $v['uid']);
                    $v['plate'] = D('Forum')->where(array('status' => 1, 'id' => $v['forum_id']))->find();
                ...
                $this->assign('navtitle', 'top');
                $this->assign('$forum', $forum);
                break;
            case 'other':
                $forum = D('ForumPost')->where(array('status' => 1,'forum_id'=>$type_id))->page($aPage, $aCount)->order('view_count desc')->select();
                foreach ($forum as &$v) {
                    $v['user'] = query_user(array('nickname', 'avatar32'), $v['uid']);
                ...
                unset($v);
                $totalCount = D('ForumPost')->where(array('status' => 1,'forum_id'=>$type_id))->count();
                $this->assign('navtitle', 'other');
                $this->assign('forum', $forum);
                break;
        }
/*分类*/
        $forum_top = D('Forum')->where(array('status' => 1))->select();        //查找顶级分类pid=0的
    ...
    public function addMoreForum()
    {
        ...
        $aType_id = I('post.forum_id', 0, 'op_t');
        $order = I('order', 'reply', 'text');
        $this->assign('order', $order);
        //取到帖子排序
        if ($order == 'ctime') {
        ...
        switch($aType){
            case 'all':
                ...
                break;
            case 'hot':
                $forum = D('ForumPost')->where(array('status' => 1))->page($aPage, $aCount)->order('view_count desc')->select();
                foreach ($forum as &$v) {
                    $v['user'] = query_user(array('nickname', 'avatar32'), $v['uid']);
            ...
            case 'top':
                ...
                break;
            case 'other':
                $forum = D('ForumPost')->where(array('status' => 1,'forum_id'=>$aType_id))->page($aPage, $aCount)->order('view_count desc')->select();
                foreach ($forum as &$v) {
                    $v['user'] = query_user(array('nickname', 'avatar32'), $v['uid']);

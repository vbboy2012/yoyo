...
class EventController extends BaseController
{
    ...
    public function index($type_id = 0, $norh = 'new')
    {
        ...
        foreach ($content as &$v) {
            ...
            $v['attend']= $this->eventAttendModel->where(array( 'event_id' => $v['id'],'status'=>1))->count();
        }
        unset($v);
       // dump($content);exit;
    ...
    public function detail($id = 0)
    {
        ...
        $attend_count= $this->eventAttendModel->where(array( 'event_id' => $id,'status'=>1))->count();
        $this->assign('attend_count', $attend_count);
        ...
        foreach ($menber as $k => $v) {
            ...
            $event_content['check_isSign'] = $this->eventAttendModel->where(array('uid' => is_login(), 'event_id' => $id))->select();
        }
...
/*            $this->checkAuth('Event/Index/edit', $content_temp['uid'], '您无该活动编辑权限。');*/
    ...
    public function doSign($event_id, $name, $phone)
    {
        ...
        if (!$check) {
            ...
            if ($res) {
                D('Message')->sendMessageWithoutCheckSelf($event_content['uid'], '报名通知', get_nickname( is_login()) . '报名参加了活动]' . $event_content['title'] . ']，请速去审核！', 'Event/Index/member', array('id' => $event_id));
    ...
    public function shenhe($uid, $event_id, $tip)
    {
        ...
        if ($tip) {
            ...
            D('Message')->sendMessageWithoutCheckSelf($uid, '审核通知', get_nickname( is_login()) . '已经通过了您对活动[' . $event_content['title'] . ']的报名请求', 'Event/Index/detail', array('id' => $event_id));
        } else {
            $this->eventModel->where(array('id' => $event_id))->setDec('attentionCount');
            D('Message')->sendMessageWithoutCheckSelf($uid, '取消审核通知', get_nickname(is_login()) . '取消了您对活动[' . $event_content['title'] . ']的报名请求', 'Event/Index/member', array('id' => $event_id));
        }
        if ($res) {
            $this->success('操作成功');
        } else {
    ...
    public function unSign($event_id)
    {
        ...
        if ($res) {
            ...
            D('Message')->sendMessageWithoutCheckSelf($event_content['uid'], '取消报名通知', get_nickname( is_login()) . '取消了对活动[' . $event_content['title'] . ']的报名', 'Event/Index/detail', array('id' => $event_id));

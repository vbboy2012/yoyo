    ...
    public function register()
    {
        ...
        switch ($aRegisteredtype) {
            case "username":
                ...
                $aNickname = I('post.unickname', '', 'op_t');
                $aPassword = I('post.upassword', '', 'op_t');
                break;
            case "email":
                $aRegType = "email";
                $aUsername = $username = I('post.email', '', 'op_t');
                $aNickname = I('post.enickname', '', 'op_t');
                $aPassword = I('post.epassword', '', 'op_t');
                break;
            case "mobile":
                $aRegType = "mobile";
...
/*                $user = UCenterMember()->where(array('username' => $username, 'status' => 1))->find();
    ...
    public function login()
    {
        ...
        if (IS_POST) {
            ...
            if ($result['status']) {
                $data['status'] = 1;
                $data['info'] = "登录成功！";
                $data['url'] = S('jump_url'.$_SERVER['REMOTE_ADDR']);
            } else {
                $data['status'] = 0;
     ...
     */
    public function createCode($uid = 0)
    {
        ...
        D('Mob/Invite')->decNumber($uid,$aTypeId,$aCanNum*$aCodeNum);//修改用户信息
        $data['can_num'] = $aCanNum;
        $data['invite_type'] = $aTypeId;
        $result = D('Mob/Invite')->createUserCode($uid,$data,$aCodeNum);
        //$this->ajaxReturn($result);
    }
}
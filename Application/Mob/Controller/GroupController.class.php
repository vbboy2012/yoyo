   ...
   // dump($id);exit;
    ...
    public function invitationFriend()
    {
        ...
        if (IS_POST) {
            ...
            $group = D('Group')->getGroup($aGroupId);
            foreach ($uids as $uid) {
                D('Message')->sendMessage($uid, '邀请加入群组', get_nickname(is_login()) . "邀请您加入群组【{$group['title']}】  <a class='ajax-post' href='" . U('Mob/Group/attend', array('group_id' => $aGroupId)) . "'>接受邀请</a>", 'Mob/Group/attend', array('id' => $aGroupId), is_login());
            }

...
class QuestionController extends BaseController
{
    ...
    public function index($mark = 0,$typeId=0)      //,待回答mark=>0，热门问题=>1，我的问题=>2，全部问题=>3
    {
        ...
        if(I('post.mark', '', 'op_t')){
            $mark=I('post.mark', '', 'op_t');
            $aKeywords = I('post.keywords', '', 'op_t');
        }
        ...
        switch ($mark) {
            ...
            case 'childType':
                ...
                break;
            case 'search':
                $questionlist=M('Question')->where(array('title'=>array('like','%' . $aKeywords . '%'),'status'=>1))->order('create_time desc')->page($aPage, $aCount)->select();
                $totalCount = D('Question')->where(array('title'=>array('like','%' . $aKeywords . '%'),'status'=>1))->count();
                $this->setTopTitle('搜索'.$aKeywords);
        }
    ...
    public function questionDetail($id = 0)
    {
        ...
        foreach ($questionComment as &$v) {
            ...
            $list = D('QuestionAnswerComment')->where('status=1 and answer_id=' . $v['id'])->select();
            foreach ($list as $k => &$c) {
                $c['userInfo'] = query_user(array('avatar128', 'nickname', 'uid', 'space_url'), $c['uid']);
                $c['content'] = parse_weibo_mobile_content($c['content']);


                $lzl_has_support=M('QuestionAnswerCommentSupport')->where(array('uid' => is_login(), 'acomment_id' => $c['id']))->find();
...
//dump($questionDetail);exit;
    ...
    private function _doEditQusetion()
    {
        ...
        if($attach_ids){
            ...
            $img_ids = implode('', $img_ids);
            //  dump($img_ids);
            $data['description']= $data['description'].$img_ids;
            $contentHandler=new ContentHandlerModel();
...
//                $postUrl = "http://$_SERVER[HTTP_HOST]" . U('Mob/Question/questionDetail',array('id'=>$aId));
                $question = D('Mob/Question')->getData($res);
    ...
    public function doSendLZLReply($p=1)
    {
        ...
        //写入数据库
        $model = D('QuestionAnswerComment');
        $data['uid']=is_login();
        $data['answer_id']=$to_f_reply_id;
        $data['content']=$content;
        $data['create_time']=time();
        $data['status']=1;
        $data['to_comment_id']=$post_id;
        $data['is_quizzer']=0;
        ...
        M('QuestionAnswer')->where(array('id'=>$to_f_reply_id))->setInc('comment_count',1);
        //显示成功页面

        exit(json_encode(array('status'=>1,'info'=>'回复成功。')));
    }

    public function dellzlreply(){
        $aReplyId=I('post.lzlreplyId', 0, 'intval');

        $res= M('question_answer_comment')->where(array('id'=>$aReplyId))->delete();
        if($res){

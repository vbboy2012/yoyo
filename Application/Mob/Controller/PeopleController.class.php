...
class  PeopleController extends BaseController
{
    public function index()
    {
        ...
        if (empty($peoples)) {
            $peoples = M('Member')->where($map)->field('uid,reg_time,last_login_time')->order('last_login_time desc')->page($aPage, 10)->select();
            foreach ($peoples as &$v) {
                $v = query_user(array( 'avatar32', 'nickname', 'title', 'uid','signature', 'rank_link','fans','following','space_mob_url'), $v['uid']);
            }
            unset($v);
        ...
        if ($aPage > 1) {
            $data['html'] = '';
            $data['status'] = 1;
            $data['html'] .= $this->fetch('_peoplelist');

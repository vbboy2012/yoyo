...
//查询不同mark对应的内容
        ...
        foreach ($type as &$v) {
           ...
           // dump($name);
            if($name=="addJob"||$name=="addHouse"||$name=="addPTJob"||$name=="addjianli"){
                $v['addurl'] = U('Mob/Cat/' . $name . "/" . $entity);
            }else{
                $v['addurl'] = U('Mob/Cat/' . 'addbase' . "/" . $entity);
            }
    ...
    public function detail($info_id = "")
    {
        ...
        $map=getUserConfigMap('user_cover','',is_login());
        $map['role_id']=0;
        $model=D('Ucenter/UserConfig');
        $cover=$model->findData($map);
        $read[$key]['user']['cover_id']=$cover['value'];
        $read[$key]['user']['cover_path']=getThumbImageById($cover['value'],1140,230);
        ...
        $model=D('Ucenter/UserConfig');
        $cover=$model->findData($map);
        $info['user']['cover_id']=$cover['value'];
        $info['user']['cover_path']=getThumbImageById($cover['value'],1140,230);
...
//其他自定义发布页面数据渲染。
    public function addBase($edit = "", $infoId = "", $entity_id = "")//$edit判断是否是编辑，默认为空，当有任何值传过来时就为编辑模式。$infoId编辑的内容ID。$entity_id，入口ID
    {
        ...
        $search_fields= D('cat_field')->where(array('entity_id'=>$entity_id,'status'=>1))->select();
    ...
    public function userContent()
    {
        ...
        $model=D('Ucenter/UserConfig');
        $cover=$model->findData($map);
        $user['cover_id']=$cover['value'];
        $user['cover_path']=getThumbImageById($cover['value'],1140,230);

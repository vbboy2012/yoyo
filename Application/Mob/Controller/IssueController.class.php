...
class IssueController extends BaseController
{
    ...
    public function index()
    {
        ...
        foreach ($issue as &$a){
            $a['support_count']=M('support')->where(array('row'=>$a['id'],'appname'=>'Issue'))->count();
            $a['support_uid']=M('support')->where(array('row'=>$a['id'],'appname'=>'Issue'))->field('uid')->select();

        }
        foreach ($issue as &$b){
            foreach($b['support_uid'] as &$c)
            {
                if($c['uid']==is_login()){
                    $b['is_support']=1;
                }else{

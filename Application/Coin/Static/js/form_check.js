...
var bind_form_check=function(){
    ...
    $('.form_check').blur(function () {
        ...
        if(obj.attr('name') == 'max_price'){
            var maxPrice = parseInt(obj.val());
            if(maxPrice == "" || maxPrice.length == 0){
                return;
            }
            var minPrice = parseInt($("input[name='min_price']").val());
            if(maxPrice <= minPrice){
                check_form.error("最大限额必须大于最小限额");
            }
        }
        if(obj.attr('name') == 'low_price'){
            var lowPrice = parseInt(obj.val());
            if(lowPrice == "" || lowPrice.length == 0){
                return;
            }
            var price = parseInt($("input[name='price']").val());
            if(lowPrice >= price){
                check_form.error("最低价必须小于报价");
            }
        }
    })
